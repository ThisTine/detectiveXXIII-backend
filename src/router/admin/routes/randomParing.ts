...
const randomParing = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { prisma } = req
        const users_first_year = await prisma.user.findMany({ where: { room_id: null } })
        ...
        // paring 3 users
        const users_with_no_pair = (await prisma.user.findMany({ where: { year: 2 }, select: { room: { include: { _count: { select: { users: true } } } }, room_id: true, id: true } }))

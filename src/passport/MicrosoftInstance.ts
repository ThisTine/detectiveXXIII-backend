...
async (accessToken:string, refreshToken:string, profile:any, done:OAuth2Strategy.VerifyCallback)=>{
        let pf:any;
    try{
        pf = await axios.get(`https://graph.microsoft.com/beta/me/photos/432x432/$value`,{headers:{Authorization:`Bearer ${accessToken}`,"Content-Type":"image/jpg"},responseType:"arraybuffer"})
    }catch(err){
        pf = {}
    }
    try{
       const {data:{onPremisesSamAccountName}} = await axios.get(`https://graph.microsoft.com/v1.0/users/${profile.id}?$select=onPremisesSamAccountName`,{headers:{Authorization:`Bearer ${accessToken}`,"Content-Type":"application/json"}})
        const { _json} = profile
        if(!onPremisesSamAccountName || !(( (onPremisesSamAccountName as string).startsWith("62") || (onPremisesSamAccountName as string).startsWith("63")|| (onPremisesSamAccountName as string).startsWith("64") || (onPremisesSamAccountName as string).startsWith("65")) && _json.officeLocation === "Computer Science") || !onPremisesSamAccountName || !_json ){
        ...
        const user = await prisma.user.upsert({
            ...
            create:{
                ...
                ...(eduyear === 2 && {room: {create:{code:nanoid(7)}}})
            },
            include: {room:true}
        })
        if(!user.room && user.year === 2){
           await prisma.user.update({where:{id:user.id},data:{room:{create:{code:nanoid(7)}}}})
        }
       return done(null,{id:user.id,email:user.email,name:user.name,year:user.year,isAdmin:user.isAdmin,isPlayable:user.isPlayable})
    }catch(err:any){
       return done(err)
    }
}
